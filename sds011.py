...
class SDS011:
    ...
    def wake(self):
        ...
        cmd = self.make_command(_SDS011_CMDS['SLEEPWAKE'],
                _SDS011_CMDS['SET'], bytes(chr(1),'utf8'))
        self._uart.write(cmd)

    def sleep(self):
        """Sends sleep command to sds011 (stops its fan)."""
        cmd = self.make_command(_SDS011_CMDS['SLEEPWAKE'],
                _SDS011_CMDS['SET'], bytes(chr(0),'utf8'))
        self._uart.write(cmd)
    ...
    def query(self):
        ...
        cmd = self.make_command(_SDS011_CMDS['QUERY'], bytes(chr(0),'utf8'), bytes(chr(0),'utf8'))
        self._uart.write(cmd)
    ...
    def process_measurement(self, packet):
        try:
            *data, checksum, tail = struct.unpack('<HHBBBs', packet)
            ...
            data_sum = sum([int(hex(packet[i]), 16) for i in range(len(packet)-2)])
